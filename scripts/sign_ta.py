...
def get_args():
	...
	parser.add_argument('--market_id', default = 'null', help = 'market id, hex format')
	parser.add_argument('--chipset_id', default = 'null', help = 'chipset id, hex format')
...
def main():
	...
	if args.market_id == 'null':
		market_id = 0
	else:
		market_id = int(args.market_id, 16)

	if args.chipset_id == 'null':
		chipset_id = 0
	else:
		chipset_id = int(args.chipset_id, 16)

	signed_hdr_obj = signed_ta_hdr(ta_hdr_info, market_id, chipset_id)
	...
	print ('                       chipset_id = ' + args.chipset_id)
